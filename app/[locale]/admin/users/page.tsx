...
export default function UsersPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/admin/users', {
        ...
        body: JSON.stringify({ email, name, password, role, producer: role === 'producer' ? producer : null }),
      });
      ...
      setMessage({ type: 'success', text: t('userCreated', { email }) });
      setEmail('');
      setName('');
      setPassword('');
      setRole('viewer');
      ...
      setMessage({
        type: 'error',
        text: err instanceof Error ? err.message : t('unknownError'),
      });
    } finally {
      setSubmitting(false);
    }
  ...
  const handleToggleActive = async (userId: number) => {
    ...
    try {
      ...
      setMessage({
        type: 'success',
        text: data.active ? t('userReactivated') : t('userDeactivated'),
      });
      await fetchUsers();
    } catch (err) {
      setMessage({
  ...
  const handleResetPassword = async (userId: number) => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: t('passwordReset') });
      setResetUserId(null);
      setResetPassword('');
    } catch (err) {
      setMessage({
    ...
    <div>
      ...
      {showForm && (
        <div className="bg-[#0e0e0e] border border-white/[0.06] rounded-xl p-6 mb-6">
          <h3 className="text-cream text-sm font-medium mb-4">{t('createUser')}</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  value={role}
                  onChange={(e) => {
                    const newRole = e.target.value as 'admin' | 'viewer' | 'producer';
                    setRole(newRole);
                    if (newRole !== 'producer') setProducer('');
                  }}
                  className="w-full bg-[#080808] border border-white/[0.08] rounded-lg px-3 py-2 text-cream text-sm focus:border-gold/40 focus:outline-none"
                >
      ...
      <div className="bg-[#0e0e0e] border border-white/[0.06] rounded-xl overflow-hidden">
        {loading ? (
          <div className="px-6 py-12 text-center">
            ...
            <p className="text-muted text-sm">{t('loadingUsers')}</p>
          </div>
        ) : users.length === 0 ? (
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          >
                            {user.role === 'admin'
                              ? t('adminBadge')
                              : user.role === 'producer'
                                ? `${t('producerBadge')} — ${user.producer ? user.producer.charAt(0).toUpperCase() + user.producer.slice(1) : ''}`
                                : t('viewerBadge')}
                          </span>
