...
export default function ReportsPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setMessage({
        type: 'success',
        text: t('reportGenerated', {
          period: `${genYear}-${String(genMonth).padStart(2, '0')}`,
  ...
  return (
    <div>
      ...
      <div className="bg-[#0e0e0e] border border-white/[0.06] rounded-xl p-6 mb-6">
        <h3 className="text-cream text-sm font-medium mb-4">{t('generateNew')}</h3>
      ...
      <div className="bg-[#0e0e0e] border border-white/[0.06] rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-white/[0.06]">
          <h3 className="text-cream text-sm font-medium">{t('allReports')}</h3>
        </div>

        {loading ? (
          <div className="px-6 py-12 text-center">
            <Loader2 className="w-6 h-6 animate-spin text-gold mx-auto mb-3" />
            <p className="text-muted text-sm">{t('loadingReports')}</p>
          </div>
        ) : reports.length === 0 ? (
          <div className="px-6 py-12 text-center">
            <FileText className="w-8 h-8 text-muted/30 mx-auto mb-3" />
            <p className="text-muted text-sm">{t('noReports')}</p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between flex-wrap gap-3">
                    ...
                    {/* Downloads — API routes, keep as plain <a> tags */}
