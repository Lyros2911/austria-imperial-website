  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        {/* Shipping Address */}
        <InfoBlock icon={MapPin} title={t('orderDetail.shippingAddress')}>
        ...
        {/* Payment */}
        <InfoBlock icon={CreditCard} title={t('orderDetail.payment')}>
          <div className="space-y-2 text-sm">
            ...
            <div className="border-t border-white/[0.06] pt-2 flex justify-between font-medium">
              <span className="text-cream">{t('orderDetail.total')}</span>
        ...
        {/* Customer */}
        <InfoBlock icon={Package} title={t('orderDetail.customer')}>
          <div className="text-sm space-y-1.5">
            ...
            <p className="text-muted text-xs">
              {order.customerId ? t('orderDetail.accountId', { id: order.customerId }) : t('orderDetail.guestOrder')}
            </p>
          </div>
        </InfoBlock>
      </div>
      ...
      <div className="bg-[#0e0e0e] border border-white/[0.06] rounded-xl overflow-hidden mb-6">
        <div className="px-5 py-4 border-b border-white/[0.06]">
          <h3 className="text-cream text-sm font-medium">{t('orderDetail.positions')}</h3>
        </div>
      ...
      <div className="bg-[#0e0e0e] border border-white/[0.06] rounded-xl overflow-hidden mb-6">
        <div className="px-5 py-4 border-b border-white/[0.06] flex items-center gap-2">
          ...
          <h3 className="text-cream text-sm font-medium">{t('orderDetail.fulfillment')}</h3>
        </div>

        {order.fulfillmentOrders.length === 0 ? (
          <div className="px-5 py-8 text-center text-muted text-sm">
            {t('orderDetail.noFulfillment')}
          </div>
        ) : (
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  {fo.trackingNumber && (
                    <span className="text-muted text-xs font-mono">
                      {t('orderDetail.tracking')}: {fo.trackingNumber}
                    </span>
                  )}
                </div>
                {fo.lastError && (
                  <p className="text-red-400 text-xs mt-1">{t('orderDetail.error')}: {fo.lastError}</p>
                )}
                {fo.retryCount > 0 && (
                  <p className="text-muted text-[10px] mt-1">
                    {t('orderDetail.retryAttempts')}: {fo.retryCount}
                  </p>
                )}
      ...
      {order.ledgerEntries.length > 0 && (
        <div className="bg-[#0e0e0e] border border-white/[0.06] rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-white/[0.06] flex items-center gap-2">
            ...
            <h3 className="text-cream text-sm font-medium">{t('orderDetail.ledgerEntries')}</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/[0.04]">
                  <th className="text-left text-[10px] text-muted uppercase px-5 py-2.5">{t('orderDetail.type')}</th>
                  <th className="text-right text-[10px] text-muted uppercase px-5 py-2.5">{t('orderDetail.revenue')}</th>
                  <th className="text-right text-[10px] text-muted uppercase px-5 py-2.5">{t('orderDetail.costs')}</th>
                  <th className="text-right text-[10px] text-muted uppercase px-5 py-2.5">{t('orderDetail.profit')}</th>
                  <th className="text-right text-[10px] text-muted uppercase px-5 py-2.5">{t('orderDetail.auryx')}</th>
                  <th className="text-right text-[10px] text-muted uppercase px-5 py-2.5">{t('orderDetail.peterCol')}</th>
                  <th className="text-right text-[10px] text-muted uppercase px-5 py-2.5">{t('orderDetail.gottfriedCol')}</th>
                  <th className="text-left text-[10px] text-muted uppercase px-5 py-2.5">{t('orderDetail.dateCol')}</th>
                </tr>
              </thead>
              <tbody>
